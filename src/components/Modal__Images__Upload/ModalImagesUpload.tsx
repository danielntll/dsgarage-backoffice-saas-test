...
const ModalImagesUpload: React.FC<ContainerProps> = ({
  ...
  const handleImagesUpload = async () => {
    ...
    const toUpload: typeImageToUpload[] = imagesToUpload.map(
      (image, index) => ({
        file: image,
    ...
    handleUploadImages(toUpload).then((uploadSuccessful) => {
          ...
          {imagesToUpload.map((image, index) => {
            ...
            return (
              ...
              >
                ...
                >
                  {" "}
                  {/* Change color based on expansion */}
                ...
                <div slot="content" className={styles.accordionContent}>
                  {" "}
                  {/* Apply styles for content */}
                  <IonItem>
                    <IonLabel position="stacked">{"ALT"}</IonLabel>
                    <IonInput
                      value={
                        imageDetails[index]?.alt || image.name.split(".")[0]
